...
public:
	...
	static ptr SigScan(const char* pattern, const char* target = xorstr_("AoE3DE.exe"))
	{

		HMODULE mod = GetModuleHandleA(NULL); // Maybe change secondary pattern to module name
		MODULEINFO info;
	...
	static ptr GetCallAddress(const char* pattern, const char* target = "AoE3DE.exe")
	{
	...
	static ptr GetInstanceAddress(const char* pattern, const char* target)
	{
		auto address = Memory::SigScan(pattern, target);
		if (!address)
		{
